...
Súčasťou zobrazenia je detailná vektorová **mapa štátnej hranice Slovenska**, názvy a značky **kľúčových miest**, zelená radarová mriežka so svetovými stranami, rozsahové kruhy a živé textové overlays.
...
- 🔄 **Karuselový režim (Carousel Mode):** Automatické striedanie dvoch hlavných režimov s používateľsky nastaviteľnou frekvenciou (napr. 5 až 300 sekúnd):
  - **Meteorologický radar (SHMÚ):** Sťahovanie a resampling najnovších CMAX snímok zrážok na mape SR.
  - **Letecký radar (ADSB):** Živé sledovanie lietadiel na zelenej radarovej mriežke.
- ✈️ **Pokročilý letecký radar:**
  - **Zelená mriežka & Svetové strany:** Klasické radarové zobrazenie s koncentrickými kruhmi a kompasovými ružicami (**N**, **E**, **S**, **W**).
  - **Indikátory na okraji displeja (Edge Indicators):** Farebné bodky na obvode displeja zobrazujúce prichádzajúce lietadlá, ktoré sa nachádzajú tesne za hranicou viditeľnosti.
  ...
  - **Inteligentné zobrazovanie trás:** Zobrazenie kódov letísk (`PRG-BTS`), ak sú dostupné v API.
  - **Zvýraznenie vojenských letov:** Vojenské transpondéry sa automaticky zobrazujú s červeným symbolom, štítkom aj obvodovou bodkou.
  - **Indikátory stúpania/klesania:** Farebné vertikálne šípky ukazujúce zmenu výšky.
- 🌧️ **Živé dáta SHMÚ:** Automatické sťahovanie najnovších kompozitných radarových snímok z open-data portálu SHMÚ.
- 🗺️ **Detailná Vektorová mapa SR & Mesta:** Vylepšený polygón štátnej hranice pre plynulú siluetu Slovenska v meteo režime spolu s dynamickým filtrom miest podľa zoomu.
...
- 🔍 **Plynulý zoom:** 5 úrovní priblíženia radaru (10 km, 25 km, 50 km, 100 km a 250 km).
- 📶 **WiFiManager & NVS:** Nastavenie Wi-Fi siete, GPS polohy, časového offsetu a **frekvencie striedania režimov v sekundách** cez webový portál s trvalým ukladaním do pamäte.
...
3. Vyberte svoju Wi-Fi sieť, zadajte heslo a doplňte parametre:
   ...
   - **Interval karuselu v sekundách** (napr. `30` pre prepínanie každých 30 sekúnd)
...
4. **Výber portu:** Vo vyskakovacom okne vyber sériový port priradený tvojmu ESP32-C3 (napr. `COM3` na Windows alebo `/dev/ttyACM0` na Linuxe/Macu) a potvrď pripojenie.
